...
public class LinkedList<DataType extends Comparable<DataType>> {
	...
	private Node getNodeAt(int idx) throws Exception {
		if (this.IsEmpty()) {
			...
			throw new Exception("Index Out Of Bound - idx");
		} else {
	...
	public DataType GetAt(int idx) throws Exception {
	...
	public void AddAt(DataType item, int idx) throws Exception {
		if (idx == 0) {
			this.AddFirst(item);
	...
	public DataType RemoveFirst() throws Exception {
		...
		DataType retval = this.head.data;
		if (this.size() == 1) {
			this.head = null;
			this.tail = null;
		} else {
	...
	public DataType RemoveLast() throws Exception {
		...
		DataType retval = this.tail.data;
		if (this.size() == 1) {
			this.head = null;
			this.tail = null;
		} else {
	...
	// Time Comlplexity O(N)
	public DataType RemoveAt(int idx) throws Exception {
		if (idx == 0) {
	...
	public DataType KthFromLast(int k) {
	...
	public LinkedList merge(LinkedList other) {
		...
		while (maintemp != null && othertemp != null) {
			if (maintemp.data.compareTo(othertemp.data) == -1) {
	...
	public void mergeSort() {
		LinkedList myanswer = this.mergeSortHelper();
	...
	private LinkedList mergeSortHelper() {
		...
		LinkedList second = new LinkedList(midpo, this.tail, this.size() / 2);
		first = first.mergeSortHelper();
		second = second.mergeSortHelper();
		LinkedList retval = first.merge(second);
		return retval;
	...
	/** It is A Generic Linked List so these Functions can't be used.... */

	// public void SortOddEven() {
	// SortOddEven(this.head, this.head.next, 1, this.size());
	// }
	// private void SortOddEven(Node first, Node second, int ci, int ti) {
	// if (ti == 2) {
	// return;
	// }
	// if (ci == ti) {
	// SortOddEven(this.head, this.head.next, 1, ti - 1);
	// return;
	// }
	// if (first.data % 2 == 0 && second.data % 2 != 0) {
	// int temp = first.data;
	// first.data = second.data;
	// second.data = temp;
	// }
	// SortOddEven(first.next, second.next, ci + 1, ti);
	// }
	//
